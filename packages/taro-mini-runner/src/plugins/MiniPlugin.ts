      ...
      compilation.hooks.normalModuleLoader.tap(PLUGIN_NAME, (_loaderContext, module: /** TaroNormalModule */ any) => {
        ...
        if (module.miniType === META_TYPE.ENTRY) {
          ...
          const loaderName = isBuildPlugin
            ? '@tarojs/taro-loader/lib/native-page'
            : isIndependent
  ...
  getAppPluginEntries () {
    ...
    Object.values(plugins).forEach((plugin) => {
      if (plugin.export) {
        ...
        this.pluginExportEntries.set(fileName, filePath.replace(path.extname(filePath), ''))
      }
    })
  }
        ...
        if (item.plugins) {
          Object.values(item.plugins).forEach((plugin) => {
            if (plugin.export) {
              ...
              const fileName = path.basename(filePath).replace(path.extname(filePath), '')
              this.pluginExportEntries.set(`${root}/${fileName}`, filePath.replace(path.extname(filePath), ''))
            }
          })
        }
      })
